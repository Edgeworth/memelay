...
use crate::ops::crossover::{crossover_arith_rand, crossover_kpx_rand};
...
impl Evaluator for Rastrigin {
    ...
    fn mutate(&self, s: &mut State, rate: f64) {
        let mut r = rand::thread_rng();
        mutate_rate(s, 1.0, |v, r| mutate_normal(v, rate, r).clamp(RANGE.start, RANGE.end), &mut r);
    }
...
pub fn rastrigin_runner(dim: usize, cfg: &Cfg) -> Runner<Rastrigin> {
    let mut r = rand::thread_rng();
